...
import {
    ...
    RefreshRoomPromptMessage, EmptyMessage, RoomsList,
        ...
        this.app.get("/rooms", { middlewares: [adminToken] }, async (req, res) => {
            try {
                ...
                for (const roomClient of roomClients) {
                    promises.push(new Promise<RoomsList>((resolve, reject) => {
                        roomClient.getRooms(emptyMessage, new Metadata(), {
                ...
                for (const roomsListResult of roomsListsResult) {
                    if (roomsListResult.status === "fulfilled") {
                        ...
                        console.warn("One back server did not respond within one second to the call to 'getRooms': ", roomsListResult.reason);
                    }
                }
